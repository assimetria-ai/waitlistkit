...
import { useEmailPreview } from '../../../hooks/@custom/useEmailPreview'
...
export function EmailPreviewPage() {
  const { templateNames, loading, error, fetchTemplateList, previewUrl } = useEmailPreview()
  const [selected, setSelected] = useState(null)
  const [iframeKey, setIframeKey] = useState(0) // bump to force reload
  const iframeRef = useRef(null)

  // Set first template as default once list loads
  if (templateNames.length > 0 && !selected) setSelected(templateNames[0])
