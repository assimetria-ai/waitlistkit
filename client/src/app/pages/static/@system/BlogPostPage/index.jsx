...
// @custom — loads via useBlogPost hook (DB-backed) with static fallback
...
import { useBlogPost } from '../../../hooks/@system/useBlog.js'
...
// ── Minimal markdown renderer (same approach) ─────────────────
...
export function BlogPostPage() {
  ...
  // Load post via hook (pages → hooks → api layer)
  const { post: apiPost, notFound, loading } = useBlogPost(slug)

  // Determine which post to display:
  // - If hook confirmed 404 and no static match → show 404 page
  // - If hook returned a post → use it
  // - If API unavailable (apiPost null, notFound false) → fall back to static
  const post =
    notFound
      ? undefined
      : apiPost !== null
